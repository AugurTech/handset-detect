...
function openJSONFile( file, tree ) {
    ...
    UAArray[filter] = subTree;
    }
    tree[ file.replace( UA_OR_JSON, '' ) ] = reduceSparceArray(UAArray);
}
...
function reduceSparceArray( sparceArray ) {
    let compactArray = [];

    for ( let i = 0; i < sparceArray.length; i++ ) {
        if ( sparceArray[i] !== undefined && sparceArray[i] !== null ) {
            compactArray.push( sparceArray[i] );
        }
    }
    return compactArray;
}
