...
require('fs').readFile(`${ __dirname }/database.json`, function( error, buffer ) {
    if ( error === null ) {
        Object.assign( TREE, JSON.parse( buffer ) );
    } else {
        console.error( error );
    }
});
...
function parse( userAgent ) {
    ...
    for ( let i = 0, j = TRAVERSAL_ORDER.length, rootNode, branch,
            ...
            filter, path, leaf, order, indexFound;
            i < j;
            i++
        ) {
                    ...
                    for ( leaf in path ) {
                        if ( normalizedUA.includes( leaf ) === true ) {
                            switch( rootNode ) {
                                ...
                                /*falls through*/
                                case '1': {
                                    parsedUserAgent = Object.assign( {}, TREE.DEVICE[ path[ leaf ] ] );
                                    break;
                                }
                                case 'browser':
                                case 'platform': {
                                    // a little bit of extra work that may be trimmed out
                                    parsedUserAgent = Object.assign( parsedUserAgent || {}, TREE.EXTRAS[ path[ leaf ] ] );
                                    break;
                                }
                            }
